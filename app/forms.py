...
class EditProfileForm(FlaskForm):
    ...
    default_region_code = StringField('Код региона (RR)', validators=[
    ...
    default_carrier_id = StringField('ID Перевозчика (TTTT)', validators=[
    ...
    default_unit_id = StringField('ID Подразделения (DDDD)', validators=[
        DataRequired(), Length(min=1, max=4), Regexp(r'^\d+$', message='ID должен содержать только цифры (максимум 4).')
    ], filters=[
        lambda x: x.zfill(4) if x else x 
    ])
